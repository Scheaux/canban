<template>
  <section class="cards-wrapper" :style="`background: ${options.color}`">
    <v-btn-toggle>
      <v-btn @click="sortListRating" title="Рейтинг (по убыванию)">
        <v-icon>mdi-sort-ascending</v-icon>
      </v-btn>
      <v-btn @click="sortListRatingReverse" title="Рейтинг (по возрастанию)">
        <v-icon>mdi-sort-descending</v-icon>
      </v-btn>
      <v-btn @click="sortList" title="Без сортировки">
        <v-icon>mdi-sort</v-icon>
      </v-btn>
    </v-btn-toggle>

    <section @drop="onDrop($event, options.id)" @dragover.prevent @dragenter.prevent>
      <div class="title">
        <h2>
          {{ options.title }}
        </h2>
      ...
      <CardItem v-for="(card, index) in cards" draggable="true" :key="index" :card="card" :options="props.options" @delete-card="deleteCard(card.id)" @dragstart="onDragStart($event, card)" />

      <CardForm title="Добавление новой карточки" v-model="isNewCardDialogOpen" :form="form" @save-card="addCard" @close-form="isNewCardDialogOpen = false" />
    </section>
  </section>
</template>
...
function sortListRatingReverse() {
  getLocalCards();
